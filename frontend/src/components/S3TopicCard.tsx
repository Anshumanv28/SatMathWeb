...
const S3TopicCard: React.FC<S3TopicCardProps> = ({ topicName, onClick }) => {
  ...
  const loadFileCount = async () => {
    try {
      ...
      console.log(`S3TopicCard: Received ${files.length} files for ${topicName}:`, files);
      
      const pdfs = files.filter(f => f.type === 'pdf');
      const docx = files.filter(f => f.type === 'docx');
      
      console.log(`S3TopicCard: ${topicName} breakdown - Total: ${files.length}, PDFs: ${pdfs.length}, DOCX: ${docx.length}`);
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600 mb-4">
              {topicName === 'geometry' && 'Shapes, angles, and spatial relationships'}
              {topicName === 'number theory' && 'Numbers, equations, and mathematical concepts'}
              {topicName === 'statistics' && 'Data analysis and probability'}
              {topicName === 'miscellenious' && 'Additional SAT math topics'}
              {topicName === 'Heart of Algebra' && 'Core algebraic concepts and functions'}
              {!['geometry', 'number theory', 'statistics', 'miscellenious', 'Heart of Algebra'].includes(topicName) && 'SAT Math topic'}
            </p>
            <div className="flex items-center text-sm text-gray-500">
              {loading ? (
                ...
                <>
                  ...
                  <span>
                    {fileCount === 0 ? 'No files available' : (
                      <>
                        {fileCount} file{fileCount !== 1 ? 's' : ''} 
                        {pdfCount! > 0 && docxCount! > 0 && (
                          <span className="text-xs text-gray-500 ml-1">
                            ({pdfCount} PDF{pdfCount !== 1 ? 's' : ''}, {docxCount} DOCX)
                          </span>
                        )}
                        {pdfCount! > 0 && docxCount === 0 && (
                          <span className="text-xs text-gray-500 ml-1">(PDFs)</span>
                        )}
                        {docxCount! > 0 && pdfCount === 0 && (
                          <span className="text-xs text-gray-500 ml-1">(DOCX)</span>
                        )}
                      </>
                    )}
                  </span>
